...
public class SearchResult {
    ...
    public Boolean openSizechart() {
        try {
            ...
            //WebElement sizeCHART=parentElement.findElement(By.xpath("//*[@id='root']/div/div/div[3]/div[1]/div[2]/div/div/div[1]/button"));
    ...
    public Boolean verifySizeChartExists() {
        ...
        try {
             ...
             */
            Thread.sleep(2000);
            WebElement sizeChartExist=parentElement.findElement(By.xpath("following-sibling::button"));
            status=sizeChartExist.getText().equals("SIZE CHART");
            return status;
        } catch (Exception e) {
            return status;
        }
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders, List<List<String>> expectedTableBody,
        ...
        try {
            ...
            // List<WebElement> tableHeader=driver.findElements(By.xpath("/html/body/div[2]/div[3]/div/div/table/thead/tr"));
            List<String> actualHeader=new ArrayList<>();
            for(WebElement headerElement: tableHeader){
                actualHeader.add(headerElement.getText());
            }
            if(!actualHeader.containsAll(expectedTableHeaders)){
                status=false;
            }
            ...
            for(int i =0; i<expectedTableBody.size();i++) {
                WebElement temp=tableBodytr.get(i).findElement(By.xpath("td"));
                for(int j=1;j<=expectedTableBody.get(i).size();j++){
                    if(!tableBodytr.get(i).findElement(By.xpath("td["+j+"]")).getText().equalsIgnoreCase(expectedTableBody.get(i).get(j-1))){
                        return false;
                    }
                }
            }
